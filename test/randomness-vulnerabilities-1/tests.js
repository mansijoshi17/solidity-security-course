...
describe("Randomness Vulnerabilites Exercise 1", function () {
  ...
  it("Exploit", async function () {
    const attackGameFacroty = await ethers.getContractFactory(
      "contracts/randomness-vulnerabilities-1/AttackGame.sol:AttackGame",
      attacker
    );
    this.attackGame = await attackGameFacroty.deploy(this.game.address);
    await this.attackGame.connect(attacker).attack();
  });
